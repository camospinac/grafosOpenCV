...
name=input("Ingrese el nombre de la imagen con extension (debe estar junto a este programa): ")
...
for i in range(len(adj)):
	...
	for j in range(len(adj[i])):
		if i==j:
			...
			dist=int(input("Cual es la transicion del estado "+str(i)+" al estado "+ str(j)+"?: "))
		else:
			dist=INF
		row.append(dist)
	C.append(row)
...
init=int(input("Cual es el nodo inicial?: "))
d[init]=0
